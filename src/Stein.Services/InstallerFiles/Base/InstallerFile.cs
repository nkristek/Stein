...
namespace Stein.Services.InstallerFiles.Base
{
        ...
        : IInstallerFile
    {
        ...
        protected async Task ReadMsiMetadata(string filePath, IMsiService msiService)
        {
            await Task.Run(() =>
            {
                using (var database = msiService.GetMsiDatabase(filePath))
                {
                    ...
                    var languageString = msiService.GetPropertyFromMsiDatabase(database, MsiPropertyName.ProductLanguage);
                    ...
                    var versionString = msiService.GetPropertyFromMsiDatabase(database, MsiPropertyName.ProductVersion);
